...
| `sync-epic-schedules.sh` | Raise open descendants of scheduled Epics to their Epic's Schedule tier (raise-only; reports children above the Epic or at Later). Dry run unless `--apply` | `bash .agents/skills/shared/sync-epic-schedules.sh [--apply] [--tiers "Focus Now Next"] [<EPIC>...]` |

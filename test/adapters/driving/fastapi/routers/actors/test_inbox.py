...
def test_store_nested_inbox_object_skips_case_ledger_entry(datalayer):
    ...
    _store_nested_inbox_object(datalayer, activity, raw_body)
    # CaseLedgerEntry must NOT be in DataLayer — CheckLedgerEntryAlreadyStored
    # uses dl.read(entry.id_) as the "fully processed" gate; pre-storing would
    # cause it to skip effects on first delivery.
    assert datalayer.read(entry.id_) is None


def test_store_nested_inbox_object_mutates_activity_object(datalayer):
    """_store_nested_inbox_object must mutate activity.object_ to the typed form.

    The re-parsed specific-type object is written back onto activity.object_ so
    that FastAPIIngressAdapter.rehydrate() can call rehydrate(activity, dl)
    without a DataLayer round-trip.  This preserves domain-specific fields
    (case_id, log_index, etc.) that would otherwise be lost via dehydration.
    """
    from vultron.core.models.case_ledger_entry import (
        CaseLedgerEntry as CoreCLE,
    )
    from vultron.wire.as2.vocab.objects.case_ledger_entry import (
        CaseLedgerEntry as WireCLE,
    )
    ...
    _store_nested_inbox_object(datalayer, activity, raw_body)
    # activity.object_ must be updated to the specifically-typed wire class
    # so that find_matching_semantics() can identify ANNOUNCE_CASE_LEDGER_ENTRY
    # without a DataLayer round-trip.
    assert isinstance(activity.object_, WireCLE)
    assert getattr(activity.object_, "case_id", None) == "urn:uuid:test-case"
    assert getattr(activity.object_, "log_index", None) == 0

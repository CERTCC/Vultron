...
def _host_actor(actor):
    ...
    ``GET /actors/`` enumerates the actors this node hosts, and under ADR-0072
...
def client_actors(datalayer):
    ...
    def _in_memory_actor_dl(actor_id: str = FastAPIPath(...)):
        ...
        canonical URI and opens *that* actor's store (ADR-0072), so a one-store
...
# (base_url + "actors/" + segment, ADR-0072) rather than by scanning a shared
...
def test_datalayer_get_actors_includes_embargo_policy(
    ...
    # The debug router is actor-scoped in its path now (ADR-0072): there is no

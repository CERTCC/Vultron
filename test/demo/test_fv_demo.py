...
def _create_case_from_offer(
    ...
    Args:
        ...
        dl: Store to seed into.  Defaults to *actor*'s own store, which is the
            ...
            module-level DataLayer to fall back on any more (ADR-0071), so
...
class TestRunTwoActorDemo:
    ...
    def test_full_workflow_succeeds(
        ...
        # *CaseActor's own* store to make it resolvable at all (ADR-0071).
        #
...
class TestDeliveryIsolation:
    ...
    def test_case_delivered_to_finder_via_inbox(self, delivery_setup):
        ...
        # is about (ADR-0071), and each of these apps hosts its actor plus any
...
def completed_workflow(
    ...
    **Per-test, not per-class.** This fixture's value is store state, and
    ...
    engine and so destroys the named in-memory databases (ADR-0071). A

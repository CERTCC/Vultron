...
def test_case_actor_spoofed_sender_fails(
    bridge, datalayer, case_actor, case_obj
):
    # Discriminating spoof test: the case IS seeded (case_obj) and the entry
    # is chain-consistent (prev_log_hash == genesis), so both the
    # missing-case path and the hash-chain check would otherwise ACCEPT and
    # persist this entry.  The only reason to reject is that the sender is not

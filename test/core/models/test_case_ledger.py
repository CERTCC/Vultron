...
class TestCaseLedgerEntry:
    ...
    def test_verify_hash_survives_pydantic_roundtrip(self):
        """``verify_hash()`` MUST return True after a model_dump / model_validate cycle.

        Replicas receive a ``HashChainLedgerRecord`` as wire data via
        ``Announce(CaseLedgerEntry)`` and reconstruct it via Pydantic
        deserialization.  ``received_at`` MUST be preserved from the wire
        payload rather than regenerated by ``default_factory``, otherwise the
        recomputed hash diverges from the authored hash and ``verify_hash()``
        returns False.  This is the canonicality contract described in
        ``_hashable_dict`` (CLP-02-008, SYNC-01-003, AC-4 from ISSUE-2252).
        """
        original = HashChainLedgerRecord(
            case_id=CASE_ID,
            log_index=0,
            object_id=OBJECT_ID,
        ...
        # Simulate wire transmission: dump to dict, then deserialize.
        roundtripped = HashChainLedgerRecord.model_validate(
            original.model_dump()
        )

        assert roundtripped.entry_hash == original.entry_hash
        assert roundtripped.received_at == original.received_at
        assert roundtripped.verify_hash(), (
            "verify_hash() returned False after Pydantic roundtrip — "

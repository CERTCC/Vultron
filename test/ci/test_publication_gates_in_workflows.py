...
Implements DOCBW-03-005, DOCBW-03-006, DOCBW-03-007 and DOCBW-03-008 from
``specs/docs-build-workflow.yaml``. A workflow that builds ``site/`` MUST run
``docs-withheld`` (the publication axis), ``docs-links`` (the reference axis)
and ``docs-legacy-urls`` (the continuity axis) against it, unconditionally, and
in ``deploy_site.yml`` each MUST sit between the build and the Pages upload.
...
GATES = {
    "docs-withheld": "DOCBW-03-005",
    "docs-links": "DOCBW-03-007",
    "docs-legacy-urls": "DOCBW-03-008",
}
GATE_COMMANDS = tuple(GATES)
...
def _local_action_steps(uses: str) -> list[dict[str, Any]]:
    ...
    runs = load_workflow(action_file).get("runs", {})
...
def test_site_building_workflow_runs_the_gate(workflow: Path, command: str):
    ...
    steps = _steps(load_workflow(workflow))
    assert _index_of(steps, command, "run") is not None, (
        ...
        f"ship what the gate exists to stop ({GATES[command]})."
    )

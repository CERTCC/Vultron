...
`vultron.wire.as2.vocab.objects.vulnerability_report`.
...
`vultron/types.py`
has `wire_activity: Any` and `wire_object: Any` fields. These leak wire types
into
...
responsible for translating the emitted domain events into the appropriate
output
...
individual use cases, because each adapter must know the exact calling
...
class CreateCaseResponse(BaseModel-derived-response-class):
    case_id: str
...
service / command-style use case layer**, where adapters interact with the core
through a uniform contract rather than bespoke function calls.

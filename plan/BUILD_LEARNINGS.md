...
### 2026-06-08 ISSUE-757 — Shared participant lookup node must support both case participant surfaces
...
- A shared BT lookup node that only scans `case_participants` regresses status
  workflows that previously used `actor_participant_index` membership checks.

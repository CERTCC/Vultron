...
Findings 1 and 2 were still live and are now filed as #3038 and #3039. Finding 5 (`_record_named_peer` / `backend.update()` without `setup()`) is tracked as #2707. Findings 3 and 4 (double PEC reset; lost `request.actor_id` fallback) no longer reproduce — `_cascade_pec_reset` now exists only in `services/embargo_lifecycle.py` with a single call site, and `case_proposal.py` now routes through `resolve_receiving_actor_id()`. The ADVISORY comment on PR #2726 was never a tracking reference (BW-07-009).

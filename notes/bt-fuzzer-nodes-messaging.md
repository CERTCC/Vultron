...
- **Factory-fn placement**: RESOLVED (ADR-0049, issue #1254) — **no
  production analog; no factory built.** Core does not model inbound protocol
  error message types (RE/EE/CE/GE) or the GI follow-up message, and has no
  emit path or dispatch semantic for them, so there is no stimulus for a
  follow-up responder. The underlying intent (surface and respond to a problem
  on a case) is served by the existing note-to-case-via-ledger path
  (`vultron/core/behaviors/note/`, SYNC-02-002) plus out-of-band comms (out of
  scope). Unresolvable inbound is dead-lettered
  (`create_store_dead_letter_tree`); protocol-invalid payloads yield an
  `InboxOutcome` of `"rejected"` — neither notifies the sender (open question,
  see ADR-0049 § More Information).

---

*This document was generated by studying the `vultron/bt` behavior tree*
*simulation code. Last updated: 2026-07-31.*

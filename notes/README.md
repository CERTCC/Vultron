...
| `codebase-structure.md` | Module reorganization candidates, enum refactoring (with target `enums.py` layout and migration path for `vultron/bt`/`vultron/case_states` enums), splitting `vultron_types.py` into individual modules, `CVDRoles` StrEnum design decision, API layer architecture (`api/v1` as thin admin adapter, `api/v2` as AS2-semantic adapter), code documentation strategy, demo script lifecycle logging (`demo_step`/`demo_check`), technical debt items (TECHDEBT-11 test dir layout, TECHDEBT-12 deprecated HTTP status constant) |

...
| `federation_ideas.md` | 2026-09-17 | Promoted to `docs/topics/future_work/federation.md` and the ten `_oq-*.md` fragments beside it (#3285, #3286). The note's `CaseActor` and "journal" vocabulary predates ADR-0088 and the Case Ledger Entry glossary terms, so keeping it active left the stale copy as the one agents load. Python stack notes were dropped in the promotion and remain here only. | None — every design concept carried over to the docs page, and each open question became an `_oq-*.md` fragment |

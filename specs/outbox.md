...
**Source**: ActivityPub specification, `specs/response-format.yaml` RF-06-001  
...
- `OX-02-002` Each outbox activity MUST conform to ActivityStreams 2.0 structure
  - See `specs/response-format.yaml` for response activity requirements
...
- **Response Format**: `specs/response-format.yaml` (response activity structure)
- **Idempotency**: `specs/idempotency.yaml` (duplicate delivery prevention)
- **Inbox Endpoint**: `specs/inbox-endpoint.yaml` (receiving delivered activities)
- **Handler Protocol**: `specs/handler-protocol.yaml` (handler generates responses)

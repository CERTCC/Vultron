...
| Wire vs. core class names | [vocabulary-registry](notes/vocabulary-registry.md) | `as_` prefix rule (ARCH-14-001); `VOCABULARY` and `WIRE_TYPE_MAP` keys are disjoint (ARCH-23-002) — `VOCABULARY` keys on the wire *class name*, `WIRE_TYPE_MAP` on the emitted `type` *value*, both derived by `wire_type_value()` and never hand-assigned (VM-01-004/007); a class sharing another's `type` value declares `_wire_type_alias` instead of letting import order pick the winner |

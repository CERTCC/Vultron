...
[notes/agents-md-structure.md](notes/agents-md-structure.md): write-up in the
nearest `notes/` or per-directory `AGENTS.md`, one line here.
...
| Domain object validation | [domain-validation](notes/domain-validation.md) | assignment and `append` bypass validation (CM-27-001, PRM-03-003); no `self` assignment in `mode="after"` validators (ARCH-21-004); silent `None` == fake `SUCCESS` (ARCH-15); `getattr` misses `ValueError`; `model_fields` is empty in `__init_subclass__` |
...
| Demo scenarios | [`vultron/demo/AGENTS.md`](vultron/demo/AGENTS.md), [event-driven-control-flow](notes/event-driven-control-flow.md) | gate steps on their cause, not script position (EDF-06, ADR-0058); protocol activity is emitted from `helpers/workflow.py`, not the scenario files |

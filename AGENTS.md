...
- Tests: `uv run pytest --tb=short > /tmp/last-test-run.log 2>&1; echo "exit: $?"; tail -5 /tmp/last-test-run.log` — run
  once; never use `tee | tail` (hides killed-run exit codes). See `.agents/skills/run-tests/SKILL.md`.

...
---
flowchart LR
    cvd["**CVD BT**<br/>(this page)"]
    vuldisco["[Discover Vulnerability](vuldisco_bt.md)"]
    cvd -->|A| vuldisco
    recv["[Receive Messages](msg_intro_bt.md)"]
    cvd -->|B| recv
    rm_bt["[Report Management](rm_bt.md)"]
    cvd -->|C| rm_bt
    em_bt["[Embargo Management](em_bt.md)"]
    cvd -->|D| em_bt

    proc_rm["[Process RM Messages](msg_rm_bt.md)"]
    recv --> proc_rm
    proc_em["[Process EM Messages](msg_em_bt.md)"]
    recv --> proc_em
    proc_cs["[Process CS Messages](msg_cs_bt.md)"]
    recv --> proc_cs
    proc_other["[Process Other Messages](msg_other_bt.md)"]
    recv --> proc_other

    rm_val["[Report Validation](rm_validation_bt.md)"]
    rm_bt --> rm_val
    rm_pri["[Report Prioritization](rm_prioritization_bt.md)"]
    rm_bt --> rm_pri
    rm_clo["[Report Closure](rm_closure_bt.md)"]
    rm_bt --> rm_clo
    do_work["[Do Work](do_work_bt.md)"]
    rm_bt --> do_work

    deploy["[Deployment](deployment_bt.md)"]
    do_work --> deploy
    fix_dev["[Fix Development](fix_dev_bt.md)"]
    do_work --> fix_dev
    reporting["[Reporting to Others](reporting_bt.md)"]
    do_work --> reporting
    pub["[Publication](publication_bt.md)"]
    do_work --> pub
    mon["[Monitor Threats](monitor_threats_bt.md)"]
    do_work --> mon
    id_assign["[CVE ID Assignment](id_assignment_bt.md)"]
    do_work --> id_assign
    acq_exp["[Acquire Exploit](acquire_exploit_bt.md)"]
    do_work --> acq_exp

    em_prop["[Propose Embargo](em_propose_bt.md)"]
    em_bt --> em_prop
    em_term["[Terminate Embargo](em_terminate_bt.md)"]
    em_bt --> em_term
    em_eval["[Evaluate Embargo](em_eval_bt.md)"]
    em_bt --> em_eval
```

### (A) Discover Vulnerability

[Discover Vulnerability](vuldisco_bt.md) models how a Participant enters the CVD process by finding a
vulnerability themselves, as a complement to receiving reports from others.

### (B) Receive Messages

[Receive Messages](msg_intro_bt.md) is the loop that processes the inbound message queue.
...
[Report Management](rm_bt.md) drives the RM state machine ($q^{rm}$) through its lifecycle.
...
- [Do Work](do_work_bt.md) — a parallel node covering the active work performed on
  *Accepted* reports ($q^{rm} \in A$):
  ...
  - [Reporting to Others](reporting_bt.md) — identifies relevant Participants and notifies them
      (emits `RS`)
  - [Publication](publication_bt.md) — manages preparation and release of advisories, fixes, and
      exploits; transitions $q^{cs} \xrightarrow{\mathbf{P}} P$
  - [Monitor Threats](monitor_threats_bt.md) — watches for attacks, public exploits, and public
      reports; may trigger embargo termination
...
### (D) Embargo Management

[Embargo Management](em_bt.md) drives the EM state machine ($q^{em}$) through negotiation,
active maintenance, and termination.

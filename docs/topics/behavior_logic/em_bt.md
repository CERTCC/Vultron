...
either stop trying or [propose](../topics/behavior_logic/em_propose_bt/) a new embargo.
...
When there is an outstanding embargo proposal ($q^{em} \in P$), we first attempt the [terminate](../topics/behavior_logic/em_terminate_bt/) task.
This task returns *Success* if there is a reason for ${q^{em} \in P \xrightarrow{r} N}$.

At this point, if there is no reason to [terminate](../topics/behavior_logic/em_terminate_bt/),
we proceed to the [evaluate](../topics/behavior_logic/em_eval_bt/) task described below.
...
If there is reason to [terminate](../topics/behavior_logic/em_terminate_bt/) the embargo, do so.
...
Again, we check to see if there is cause to [terminate](../topics/behavior_logic/em_terminate_bt/) doing so, if needed.
If termination is not indicated, we proceed once again to [evaluate the proposed revision](../topics/behavior_logic/em_eval_bt/), either accepting

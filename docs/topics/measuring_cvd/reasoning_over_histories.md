...
This will provide a baseline expectation about [CS events](../process_models/cs/transitions.md#cs-input-symbols).
...
!!! note "Frequency of a History"
    ...
    The frequency of a history $f_h$ is
...
the six-element histories as uniformly distributed.

!!! example "Uniform Distribution over Histories vs Transitions"
...
Each of the event pair orderings in the Event Order
Table (reproduced below from [Desirable Histories](./desirable_histories.md)
can be treated as a Boolean condition that either holds or does not hold in any given history.

{% include-markdown "./_ordered_pairs_events.md" %}

Above, we described how to compute
...
($\sigma_i \prec \sigma_j$) across all possible histories $H$.

!!! note "Formalizing Event Order Frequency"

    The following equations define the frequency of an ordering $f_{\sigma_i \prec \sigma_j}$ as the
    sum over all histories in which the ordering occurs ($H^{\sigma_i \prec \sigma_j}$) of the frequency of each such history
    ($f_h$) as shown in the [Possible Histories table](./possible_histories.md).
...
!!! example "Some event orderings have higher expected frequencies than others"
    ...
    For example, vendor awareness precedes attacks
...
!!! info inline end "Hasse Diagram for the Partial Order on Desiderata"

    The full Hasse Diagram for the partial order
    $(\mathbb{D},\leq_{\mathbb{D}})$ is shown below.
    The most skillful (lowest frequency) orderings are at the top of the diagram, while the
    least skillful (highest frequency) ones are at the bottom.
...
!!! example "Skill is inversely proportional to luck"
    ...
    As an example, from the Event Frequency
...
!!! note "A Partial Order on Desiderata"

    We create the partial order on $\mathbb{D}$ as follows: for any
    ...
    Note that the inequalities on the left and right sides of the above are flipped because skill is inversely
    proportional to luck. 
    Also, while $\leq_{\mathbb{D}}$ on the left side of the above defines a preorder over the poset
...
contained therein. The rank is shown in the [Possible Histories table](./possible_histories.md),
reproduced below for convenience.

!!! info "The Possible Histories of CVD, Ranked by Skill"

    The table below shows the possible histories of CVD, ranked by skill.
    The rank is ordered from least skillful to most skillful histories.
    Rank values start at 1 for least
    ...
    See [Possible Histories table](./possible_histories.md) for the development of the table.
...
Histories having identical rank are incomparable to each other within the poset. The
...
!!! tip "Towards a Total Order on all Possible CVD Histories ($\mathcal{H}$)"
    ...
    the branches in the Desiderata Hasse Diagram above.
    Achieving a total order on $\mathbb{D}$ would
    require determining a preference for one of each of the following:
    
    |          Outcome A           |           Outcome B           |                                          Explanation                                  |
    |:-----------------------------:|:-----------------------------:|:------------------------------------------------------------------------------------:|
    | $\mathbf{F} \prec \mathbf{X}$ | $\mathbf{V} \prec \mathbf{P}$ | fix ready before exploit publication _or_ vendor awareness before public awareness   |
    | $\mathbf{P} \prec \mathbf{X}$ | $\mathbf{X} \prec \mathbf{A}$ | public awareness before exploit publication _or_ exploit publication before attacks  |
    | $\mathbf{P} \prec \mathbf{A}$ | $\mathbf{V} \prec \mathbf{X}$ |     public awareness before attacks _or_ vendor awareness before exploit publication |

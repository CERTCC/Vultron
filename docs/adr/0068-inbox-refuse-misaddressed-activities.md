...
Each Vultron actor has its own isolated DataLayer (ADR-0071, ADR-0012). The
...
- Per-actor DataLayer isolation is only trustworthy if the store's owner was
  the intended recipient of every Activity it contains (ADR-0071).
...
- It prevents writing a misaddressed Activity to the wrong actor's store at
  ...
  exactly the cross-actor contamination ADR-0071 removes.

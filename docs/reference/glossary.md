...
| **RSVP Deadline** | The activity-level `end_time` on an `Invite(EmbargoEvent)`, giving the invitee an explicit respond-by instant after which the invitation is no longer open. Distinct from the nested `Invite.object_.end_time`, which is when the **Embargo** itself ends — the same invitation carries both, one nesting level apart. Enforced lazily by the **CASE_MANAGER** (CM-27-003); a late **Accept** is never refused outright (EMB-17). Introduced by ADR-0065 | Invite expiry, respond-by deadline, invite end_time |
...
| **HandlerResult** | The **UseCaseResult** subtype returned by received-side use cases (processing an inbound Activity); carries a **HandlerDisposition** and, when refused, a reason. Its purpose is to route the handler's own verdict to **InboxOutcome**, which is otherwise assembled from behavior-tree bookkeeping and cannot see it (ADR-0094, #2255). Not yet implemented. | Handler output, received-side result |
| **HandlerDisposition** | The closed `StrEnum` vocabulary on a **HandlerResult**: `APPLIED` (local state changed), `SKIPPED` (correct no-op — duplicate or already-present), `REFUSED` (inbound assertion rejected). `APPLIED` and `SKIPPED` both map to `InboxOutcome.status == "processed"`; `REFUSED` maps to `"rejected"`. There is deliberately no `DEFERRED` — deferral is decided by `DeferCheckNode` before dispatch, so a handler can never produce it (ADR-0094). Not yet implemented. | Handler status, handler outcome |

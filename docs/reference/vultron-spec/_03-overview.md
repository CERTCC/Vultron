...
enforces single-writer authority over shared state (§5.4.1) and replicates
...
satisfied the embargo consent gate (§9.7).
...
a triggering PXA observation — starts the teardown process (§10.1).
...
was set, or one that declined). Cascade rules keep the two in sync (§10).
...
**Two distinct role categories apply** (see §12.3 for the full taxonomy):
...
(§11.1). An actor MUST NOT self-assign a role to a case it did not initiate.

...
- **CLI scripts** (`[project.scripts]` in `pyproject.toml`):
  ...
  - `docs-level-order` → `vultron.metadata.docs.level_order:main`. Fails when a `docs/` page uses a glossary term, without linking to it first, that a higher-level page declares in `introduces:` (DF-11-002); working-record pages and pages with no level are skipped, include fragments are checked at their lowest host's level, and pre-existing violations sit in a shrink-only baseline where each entry names a reason; `--prune-baseline` drops entries that are no longer violations.

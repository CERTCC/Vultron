...
| `vultron/semantic_registry.py` is a 783-line centralized dispatch table | Single module consolidates all message-type to handler wiring | `vultron/semantic_registry.py` | Any new message type requires editing this file; tight coupling can cause merge conflicts | Tracked in GitHub concern #508 — split into domain sub-modules under `vultron/semantic_registry/` |

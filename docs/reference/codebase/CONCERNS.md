...
| High churn in planning and guidance files | Planning/spec docs evolve rapidly during active development | `plan/IMPLEMENTATION_NOTES.md` (286 changes), `AGENTS.md` (79) in the latest scan window | Agent guidance and task context can go stale quickly | Treat `plan/` and guidance docs as volatile areas during onboarding; re-read before each session |
...
|------|-------------|-------------|----------------------|
| `plan/` documentation set | Planning/history docs change very frequently | `plan/IMPLEMENTATION_NOTES.md` tops the latest churn output | Read current files immediately before editing; expect stale assumptions |

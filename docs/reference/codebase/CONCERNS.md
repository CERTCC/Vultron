...
| High churn in planning and specification files | Planning/spec docs evolve rapidly during active development | `plan/IMPLEMENTATION_PLAN.md` (386 changes), `plan/IMPLEMENTATION_NOTES.md` (306), `plan/BUGS.md` (122), `AGENTS.md` (87) in last 90 days | Agent guidance and task context can go stale quickly | Treat `plan/` and guidance docs as volatile areas during onboarding; re-read before each session |
...
|------|-------------|-------------|----------------------|
| `plan/` documentation set | Planning/history docs change very frequently | `plan/IMPLEMENTATION_PLAN.md`, `plan/IMPLEMENTATION_NOTES.md`, `plan/BUGS.md` top the 90-day churn list | Read current files immediately before editing; expect stale assumptions |

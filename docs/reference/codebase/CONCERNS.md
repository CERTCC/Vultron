...
|----------|---------|----------|--------|------------------|
| high | Demo helpers still depend on `requests`, but `requests` is not declared in `project.dependencies` | `vultron/demo/utils.py`, `vultron/demo/helpers/verification.py`, `pyproject.toml` | Fresh non-dev installs may fail when demo or verification helpers make HTTP calls; `httpx` (`>=0.28.1`) is already a declared runtime dep | Migrate demo HTTP helpers from `requests` to `httpx` or declare `requests` explicitly |
...
| Documentation/code drift around architecture targets | Notes describe both target and current structure; new ASGIEmitter rules now live in a separate note | `notes/architecture-ports-and-adapters.md`, `notes/asgi-emitter.md`, `AGENTS.md` | New contributors may confuse target layout with what exists today | Keep onboarding docs explicitly split into "current reality" vs "target direction" and cite the ASGIEmitter note where relevant |
| High churn in planning and guidance files | Planning/spec docs evolve rapidly during active development | `plan/IMPLEMENTATION_PLAN.md` (375 changes), `plan/IMPLEMENTATION_NOTES.md` (286), `AGENTS.md` (79) in the latest scan window | Agent guidance and task context can go stale quickly | Treat `plan/` and guidance docs as volatile areas during onboarding; re-read before each session |
...
|------|-------------|-------------|----------------------|
| `plan/` documentation set | Planning/history docs change very frequently | `plan/IMPLEMENTATION_PLAN.md`, `plan/IMPLEMENTATION_NOTES.md` top the latest churn output | Read current files immediately before editing; expect stale assumptions |
...
| `vultron/core/use_cases/triggers/embargo.py` | Embargo use-case trigger logic; 31 churn hits at 792 lines | 31 churn hits in the latest scan window | Narrow PRs with explicit test coverage for each embargo state transition |

### 6) Open Questions

1. Demo HTTP helpers still use `requests`, while the declared runtime HTTP
   client is `httpx`. Decide whether to migrate the helpers or make the extra
   dependency explicit.

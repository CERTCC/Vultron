...
def get_actors():
    ...
    Under ADR-0071 this enumerates hosted actors and reads each one's record
...
def create_actor(request: ActorCreateRequest, http_request: Request):
    ...
    serving endpoint the same string by construction (ADR-0071 decision 2).
    ...
    know (ADR-0071 decision 5). It is canonicalized to this node's namespace
...
def get_actor_inbox(
    ...
    # the injected DataLayer already *is* this actor's store (ADR-0071).

...
def actor_slug(actor_id: str) -> str:
    ...
    without a separate registry (ADR-0071).

    Two actors hosted by one node cannot share a slug, because under ADR-0071
...
def actor_db_url(db_url: str, actor_id: str) -> str:
    ...
    Under ADR-0071 every actor gets its own store, so the configured

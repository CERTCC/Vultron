...
_ShouldStayInRmDeferred = sequence_node("_ShouldStayInRmDeferred", """This node represents the process of deciding whether to stay in the RMDeferred state.
    ...
    """, RMinStateDeferred, NoNewDeploymentInfo)


_DecideToAcceptDeploymentTasking = sequence_node("_DecideToAcceptDeploymentTasking", """This node represents the process of deciding whether to accept a deployment tasking.
    Steps:
    ...
    """, PrioritizeDeployment, q_rm_to_A, EmitRA)


_DeferDeploymentTasking = sequence_node("_DeferDeploymentTasking", """This node represents the process of deferring a deployment tasking.
    Steps:
...
_DecideWhetherToDeploy = fallback_node("_DecideWhetherToDeploy", """This node represents the process of deciding whether to deploy a fix.
    ...
    """, RMinStateDeferred, RMinStateAccepted, _DecideToAcceptDeploymentTasking, _DeferDeploymentTasking)


_DecideAbilityToDeploy = fallback_node("_DecideAbilityToDeploy", """This node represents the process of deciding whether the deployer is able to deploy a fix.
    ...
    """, RoleIsVendor, CSinStateNotDeployedButPublicAware)


_DeployFixWhenReady = sequence_node("_DeployFixWhenReady", """This node represents the process of deploying a fix.
    Steps:
    ...
    """, _DecideAbilityToDeploy, CSinStateVendorAwareFixReadyFixNotDeployed, DeployFix, q_cs_to_D, EmitCD)


_DeployMitigationWhenReady = sequence_node("_DeployMitigationWhenReady", """This node represents the process of deploying a mitigation.
    ...
    """, MitigationAvailable, DeployMitigation)


_Deploy = fallback_node("_Deploy", """This node represents the process of deploying a fix or mitigation.
    ...
    """, RMinStateDeferred, CSinStateFixDeployed, _DeployFixWhenReady, MitigationDeployed, _DeployMitigationWhenReady)


_DeployIfDesired = sequence_node("_DeployIfDesired", """This node represents the process of deciding whether to deploy a fix or mitigation.
    ...
    """, RoleIsDeployer, _DecideWhetherToDeploy, _Deploy)


_MonitorDeploymentIfDesired = sequence_node("_MonitorDeploymentIfDesired", """This node represents the process of deciding whether to monitor a deployment and then monitoring it.
    It starts with a check that the deployment is supposed to be monitored.
    Then it performs the monitoring.
    """, MonitoringRequirement, MonitorDeployment)


Deployment = fallback_node("Deployment",
                           """This bt represents the process of deploying a fix or mitigation and monitoring the deployment.""",
                           CSinStateFixDeployed, _ShouldStayInRmDeferred, _DeployIfDesired, _MonitorDeploymentIfDesired)

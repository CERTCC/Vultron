...
_HandleRe = sequence_node("_HandleRe", """This is a stub for handling an RE message.
    Steps:
    1. Check that the message is an RE message.
    2. Follow up on what the RE message means.
    """, IsMsgTypeRE, FollowUpOnErrorMessage)


_LeaveRmStart = fallback_node("_LeaveRmStart", """Leave the RM start state by transitioning to the RECEIVED state.""",
                              RMnotInStateStart, q_rm_to_R)


_SetVendorAware = sequence_node("_SetVendorAware", """Set the case state to vendor aware.
    Steps:
    ...
    """, CSinStateVendorAware, q_cs_to_V, EmitCV)


_RecognizeVendorNotifiedIfNecessary = fallback_node("_RecognizeVendorNotifiedIfNecessary", """If we're a vendor, recognize that we've been notified.
    Short-circuits if we're not a vendor.
    """, RoleIsNotVendor, _SetVendorAware)


_HandleRs = sequence_node("_HandleRs", """Handle an RS message. The RS message type indicates an incoming report.
    Steps:
    ...
    """, IsMsgTypeRS, _LeaveRmStart, _RecognizeVendorNotifiedIfNecessary)


_ChooseRmMsgReaction = fallback_node("_ChooseRmMsgReaction", """Choose the appropriate reaction to an RM message.
    ...
    """, _HandleRs, _HandleRe, RMnotInStateStart)


_HandleAckableRmMessage = sequence_node("_HandleAckableRmMessage", """Handle an RM message that expects an ACK.
    Steps:
    ...
    """, _ChooseRmMsgReaction, EmitRK)


_HandleRmMessage = fallback_node("_HandleRmMessage", """Handle an RM message.
    Steps:
    ...
    """, IsMsgTypeRK, _HandleAckableRmMessage, EmitRE)


ProcessRMMessagesBt = sequence_node("ProcessRMMessagesBt", """Behavior tree for processing RM messages.""", IsRMMessage,
                                    _HandleRmMessage)

...
_SEMANTICS_HANDLERS_CACHE: dict[MessageSemantics, BehaviorHandler] | None = (
    None
)


def get_semantics_handlers() -> dict[MessageSemantics, BehaviorHandler]:
    """
    Returns the mapping of MessageSemantics to handler functions.
    Uses lazy initialization to avoid circular imports.
    """
    global _SEMANTICS_HANDLERS_CACHE

    if _SEMANTICS_HANDLERS_CACHE is not None:
        return _SEMANTICS_HANDLERS_CACHE

    # Import handlers lazily to avoid circular import
    from vultron.api.v2.backend import handlers as h

    _SEMANTICS_HANDLERS_CACHE = {

...
def _find_embargo_proposal(case_id: str, dl: DataLayer):
    """
...
def trigger_propose_embargo(
    ...
    The local actor decides to propose an embargo on the given case.
    Creates an EmbargoEvent, emits EmProposeEmbargo, updates local EM state,
    adds to actor outbox, and returns HTTP 202.

    EM state transitions per em_propose_bt.md:
    - EM.N → EM.P (first proposal; emits EP)
    ...
    if body.end_time is not None:
        embargo_kwargs["end_time"] = body.end_time
    ...
    _add_activity_to_outbox(actor_id, proposal.as_id, dl)
...
def trigger_evaluate_embargo(
    ...
    The local actor accepts the current embargo proposal. Emits
    EmAcceptEmbargo (Accept(EmProposeEmbargo)), activates the embargo
    on the case (EM → ACTIVE), adds to actor outbox, and returns HTTP 202.

    If proposal_id is omitted, the first pending EmProposeEmbargo for the
    case is used. Returns HTTP 404 if no proposal is found.
    ...
    if body.proposal_id:
        proposal_raw = dl.read(body.proposal_id)
        if proposal_raw is None:
            raise _not_found("EmbargoProposal", body.proposal_id)
        proposal = proposal_raw
    else:
        proposal = _find_embargo_proposal(case.as_id, dl)
        if proposal is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail={
    ...
    _add_activity_to_outbox(actor_id, accept.as_id, dl)
...
def trigger_terminate_embargo(
    ...
    The local actor announces the termination of the active embargo.
    Emits AnnounceEmbargo (ET message), sets case EM state to EXITED,
    clears the active embargo, adds to actor outbox, and returns HTTP 202.

    Returns HTTP 409 if the case has no active embargo to terminate.
    ...
    _add_activity_to_outbox(actor_id, announce.as_id, dl)

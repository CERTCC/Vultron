...
class DataLayerClient(BaseModel):
    ...
    ADR-0071: a container hosts an actor plus the CaseActors it self-hosts
    ...
    def dl_path(self, key: str = "", actor_id: str | None = None) -> str:
        ...
        Returns:
            ...
            URI from its own base URL (ADR-0071), which is why the short segment
        ...
        if not actor:
            raise ValueError(
                ...
                "are per-actor (ADR-0071). Set actor_id on the client or pass "
                "it explicitly."
            )
...
def reset_datalayer(client: DataLayerClient) -> dict:
    ...
    # *not* go through `dl_path` (ADR-0071 moved it to /admin/).
...
def verify_object_stored(
    ...
    Args:
        ...
        actor_id: Whose replica to look in.  Defaults to *client*'s own actor.
            "Is this object stored?" has no answer under ADR-0071 without naming
...
#: ``{base_url}actors/{slug}`` (ADR-0071 decision 2), so the id names the very
...
def setup_clean_environment(
    ...
    leaves it hosting nothing at all: under ADR-0071 there is no store that
...
def seed_case_actor_for_report(
    ...
    store, since the route opens the store the id names (ADR-0071).

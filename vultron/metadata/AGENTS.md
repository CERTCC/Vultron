...
`docs-links`, `glossary-index`, `learnings-index`, and `spec-backstop` console
...
`docs/links.py` read the **built** `site/` tree, so they only run after
`mkdocs build` — the two tools here whose input is a build product rather than a
source file. Both locate it through `base.built_site_dir()`, which fails rather
than passing when that input is missing (DOCBW-03-005, DOCBW-03-007, DF-09-009).
...
| `docs/` | `git log` over `docs/`, for the what's-new page; `docs/reference/glossary.md` (`glossary_index.py`); the built `site/` tree (`withheld.py`, `links.py`); every `docs/**/*.md` page's `stakeholder_type`/`level` (`page_frontmatter.py`, DF-11) | `PageFrontmatter`, `WorkingRecordFrontmatter` (`page_schema.py`); publication axis: DOCBW-03-005; reference axis: DOCBW-03-007 |

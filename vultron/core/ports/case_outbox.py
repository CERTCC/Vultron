...
number of use cases and BT nodes that also call ``record_outbox_item`` or
``outbox_append``.  Declaring ``CaseOutboxPersistence`` on a ``Received``
use case is an architectural smell — it signals that the handler mixes inbound
processing with outbound broadcast.

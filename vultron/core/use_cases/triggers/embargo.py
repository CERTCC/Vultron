...
"""
Domain use-case functions for embargo-level trigger behaviors.
...
def svc_propose_embargo(
    ...
    Raises:
        ...
        VultronConflictError: if the case EM state is EXITED.
    """
...
def svc_evaluate_embargo(
    ...
    Raises:
        VultronNotFoundError: if actor, case, or proposal cannot be resolved.
        VultronValidationError: if the proposal is not an Invite or is missing
            an embargo event reference.
    """
    actor = resolve_actor(actor_id, dl)
    actor_id = actor.as_id
    ...
    if proposal_id:
        ...
        if proposal is None:
            raise VultronNotFoundError(
                "EmbargoProposal", f"(pending for case '{case.as_id}')"
            )
...
def svc_terminate_embargo(actor_id: str, case_id: str, dl: DataLayer) -> dict:
    ...
    Raises:
        VultronNotFoundError: if actor or case cannot be resolved.
        VultronConflictError: if the case has no active embargo.
    """
    actor = resolve_actor(actor_id, dl)
    actor_id = actor.as_id

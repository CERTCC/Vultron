...
"""
Domain use-case functions for report-level trigger behaviors.

Each function accepts domain parameters and a DataLayer instance.
No HTTP framework imports (FastAPI, Starlette) are permitted here.
Callers that need HTTP error responses must catch the domain exceptions
and translate them.
"""
...
def svc_validate_report(
    ...
    Raises:
        VultronNotFoundError: if actor or offer cannot be resolved.
        VultronValidationError: if the offer does not reference a VulnerabilityReport.
    """
...
def svc_close_report(
    ...
    Raises:
        VultronNotFoundError: if actor or offer cannot be resolved.
        VultronConflictError: if the report is already CLOSED.
        VultronValidationError: if the offer does not reference a VulnerabilityReport.
    """
    actor = resolve_actor(actor_id, dl)
    actor_id = actor.as_id

...
class AddNoteToCaseReceivedUseCase:
    ...
    def execute(self) -> None:
        ...
        # Only commit canonical ledger entry if we have received-actor proof
        # (i.e., receiving_actor_id is set on the event).
        if request.receiving_actor_id is None:
            logger.debug(
                "add_note_to_case: no receiving_actor_id for case '%s'"
        ...
        if (
            case_actor_id is None
            or request.receiving_actor_id != case_actor_id
        ):
            logger.debug(
